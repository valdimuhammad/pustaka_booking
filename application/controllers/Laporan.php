...
class Laporan extends CI_Controller
{
	...
	public function laporan_member()
	{
		$data = array(
			...
			'user' => $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array(),
			'laporan' => $this->db->query(
				'SELECT * FROM member p, detail_member d, buku b, user u WHERE d.id_buku=b.id AND p.id_user=u.id AND p.no_member=d.no_member'
			)->result_array()
		);
	...
	public function laporan_member_pdf()
	{
		...
		$this->load->view('member/laporan-pdf-member', $data);
	...
	public function export_excel_member()
	{
		$data = array(
			'title' => 'Laporan Data Peminjaman Buku',
			'laporan' => $this->db->query("SELECT * FROM member p,detail_member d, buku b,user u WHERE d.id_buku=b.id AND p.id_user=u.id AND p.no_member=d.no_member")->result_array()
		);
